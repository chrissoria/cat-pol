#!/usr/bin/env python3
"""Scraper: Berkeley ordinances from municipal.codes/enactments → HuggingFace.

Uses Playwright (visible browser) to bypass Cloudflare protection on municipal.codes.

Usage:
    python scripts/build_berkeley_dataset.py
    python scripts/build_berkeley_dataset.py --start-page 1 --end-page 10
    python scripts/build_berkeley_dataset.py --push-only
...
def scrape_all_pages(
    ...
    scraped_pages_file = CHECKPOINT_DIR / "berkeley_scraped_pages.txt"
    ...
    for cp_file in sorted(CHECKPOINT_DIR.glob("berkeley_batch_*.parquet")):
    ...
    with sync_playwright() as pw:
        ...
        for page_num in range(start_page, end_page + 1):
            ...
            if len(scraped_pages) % checkpoint_interval == 0 and batch_rows:
                ...
                cp_name = f"berkeley_batch_{len(scraped_pages):05d}.parquet"
    ...
    if batch_rows:
        batch_df = pd.DataFrame(batch_rows)
        cp_name = "berkeley_batch_final.parquet"
        batch_df.to_parquet(CHECKPOINT_DIR / cp_name, index=False)
        all_rows.extend(batch_rows)
...
def merge_all_checkpoints() -> pd.DataFrame:
    ...
    if not dfs:
        print("No Berkeley checkpoint files found.")
        return pd.DataFrame()
...
def main():
    parser = argparse.ArgumentParser(description="Build Berkeley ordinances dataset")
    ...
    parser.add_argument("--repo-id", default="chrissoria/berkeley-ordinances")
    ...
    final_path = CHECKPOINT_DIR / "berkeley_final_dataset.parquet"
