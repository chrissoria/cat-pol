...
### Changed
- **`extract()` and `explore()`** now pass `domain="policy"` to `catstack.extract()`
  and `catstack.explore()`, selecting policy-specific prompt templates (uses
  "documents" / "provisions" language in the semantic merge step). Requires
  `cat-stack >= 1.0.20`.
